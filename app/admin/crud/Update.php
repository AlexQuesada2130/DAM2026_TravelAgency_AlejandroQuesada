...
if (!isset($_SESSION['es_admin']) || $_SESSION['es_admin'] != 1) { header("Location: ../../public/authentication/Login.php"); exit(); }

$db = new Database();
$conn = $db->getConnection();
$id_viaje = $_GET['id'] ?? null;

if (!$id_viaje) { header("Location: Read.php"); exit(); }
...
<head>
    ...
    <style>
        /* CSS INLINE PARA REPLICAR ESTILO */
        body { background-color: #2388C7; min-height: 100vh; display: flex; justify-content: center; align-items: center; font-family: 'Segoe UI', sans-serif; margin:0;}
        .dashboard-wrapper { background: white; padding: 40px; border-radius: 12px; max-width: 800px; width: 90%; box-shadow: 0 10px 25px rgba(0,0,0,0.2); }
        .auth-input { width: 100%; padding: 10px; margin-bottom: 10px; border: 1px solid #ccc; border-radius: 5px; box-sizing: border-box; }
        .form-label { display: block; margin-bottom: 5px; color: #333; font-weight: bold; font-size: 0.9em; }
        .form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        .full-width { grid-column: span 2; }
        .btn-save { background-color: #ffc107; color: #333; padding: 12px 20px; border: none; border-radius: 50px; cursor: pointer; font-weight: bold; }
        .btn-cancel { background-color: #6c757d; color: white; padding: 12px 20px; text-decoration: none; border-radius: 50px; font-weight: bold; }
    </style>
</head>
<body>
